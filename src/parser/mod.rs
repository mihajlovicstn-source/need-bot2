...
pub fn parse_trade(
    ...
    let signatures = match &transaction.transaction.transaction {
        EncodedTransaction::Json(transaction) => transaction.signatures.clone(),
        EncodedTransaction::Accounts(transaction) => transaction.signatures.clone(),
        EncodedTransaction::LegacyBinary(transaction) => transaction.signatures.clone(),
        EncodedTransaction::Binary(transaction) => transaction.signatures.clone(),
    };

    let signature_string = signatures.first().cloned().unwrap_or_else(|| signature.to_string());
